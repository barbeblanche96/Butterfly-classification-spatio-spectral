...
def scores_row(wiegths, total_pix, domain, image_file, species, refl_by_bands, distribution_mean_1, distribution_std_1, distribution_mean_4, distribution_std_4, distribution_mean_7, distribution_std_7, distribution_mean_10, distribution_std_10):    
    ...
    for idx in range(len(espece_pdf_1)):
        ...
        #species_proba = np.sum(norm_proba.reshape(norm_proba.shape[0], -1, 4), axis=2)
    ...
    for k, (value, key) in enumerate(zip(zscore_by_band, refl_by_bands)):
    
    
        # Global matrice
        zscore_global = zscore_by_band[k]
        
        #zs_final_score1, zs_final_score4, zs_final_score7, zs_final_score10 = dist_species = np.sum(zscore_global, axis=0)
        dist_species = np.sum(zscore_global, axis=0)
    
        
        # Global matrice
        probabilities_global = probabilities_by_band[k]
        
        #pdf_final_score1, pdf_final_score4, pdf_final_score7, pdf_final_score10 = proba_species = np.sum(np.log10(probabilities_global), axis=0)
        ...
        final_score1, final_score4, final_score7, final_score10 = scores = (wiegths[0] * (prob_q)) + (wiegths[1] * (zs_q))
        
        winner_specie_k = np.argmin(scores)
