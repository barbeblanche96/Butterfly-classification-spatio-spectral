...
# Z-score
    ...
    espece_zscore_1 = zscore_by_specie(distribution_mean_1, distribution_std_1, refl_by_bands, 'extend')
    espece_zscore_4 = zscore_by_specie(distribution_mean_4, distribution_std_4, refl_by_bands, 'extend')
    espece_zscore_7 = zscore_by_specie(distribution_mean_7, distribution_std_7, refl_by_bands, 'extend')
    espece_zscore_10 = zscore_by_specie(distribution_mean_10, distribution_std_10, refl_by_bands, 'extend')
    
    
    score_zscore_1 = np.zeros(len(espece_zscore_1))
    score_zscore_4 = np.zeros(len(espece_zscore_4))
    score_zscore_7 = np.zeros(len(espece_zscore_7))
    score_zscore_10 = np.zeros(len(espece_zscore_10))
    
    for j in range (len(espece_zscore_1)):
        idx_min = np.argmin(np.array([espece_zscore_1[j], espece_zscore_4[j], espece_zscore_7[j], espece_zscore_10[j]]))
        if (idx_min == 0):
            score_zscore_1[j] = 1
        elif (idx_min == 1):
            score_zscore_4[j] = 1
    ...
    espece_pdf_1 = pdf_with_vote_by_specie(distribution_mean_1, distribution_std_1, refl_by_bands, 1/14, 'extend')
    espece_pdf_4 = pdf_with_vote_by_specie(distribution_mean_4, distribution_std_4, refl_by_bands, 1/14, 'extend')
    espece_pdf_7 = pdf_with_vote_by_specie(distribution_mean_7, distribution_std_7, refl_by_bands, 1/14, 'extend')
    espece_pdf_10 = pdf_with_vote_by_specie(distribution_mean_10, distribution_std_10, refl_by_bands, 1/14, 'extend')
    
    score_pdf_1 = np.zeros(len(espece_pdf_1))
    score_pdf_4 = np.zeros(len(espece_pdf_4))
    score_pdf_7 = np.zeros(len(espece_pdf_7))
    score_pdf_10 = np.zeros(len(espece_pdf_10))
    
    for j in range (len(espece_pdf_1)):
        idx_max = np.argmax(np.array([espece_pdf_1[j], espece_pdf_4[j], espece_pdf_7[j], espece_pdf_10[j]]))
        if (idx_max == 0):
            score_pdf_1[j] = 1
        elif (idx_max == 1):
            score_pdf_4[j] = 1
    ...
    winner_specie =  np.argmax([final_score1, final_score4, final_score7, final_score10])
    winner = None
